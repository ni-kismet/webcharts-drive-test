    ...
    function init(plot) {
                ...
                visibleCursors(cursors).forEach(function(cursor) {
                    ...
                    cursor.thumbs.forEach(function (thumb) {
                        if (thumb === e.detail.target) {
                            targetCursor = cursor;
                            dragmode = (xAxisThumb(cursor) && thumbIndex === 0) ? 'x' : 'y';
                        }
                    });
                });
        ...
        function createThumbs(cursor, plot, plotOffset) {
            ...
            if (xAxisThumb(cursor)) {
                ...
                thumbCy = cursor.showThumbs.indexOf('b') !== -1 ? plot.height() + plotOffset.top + plotOffset.bottom - thumbRadius : plotOffset.top - thumbRadius;
                ...
                thumb = $.thumb.createThumb(thumbRadius, thumbCx, thumbCy, svgRoot, constraintFunction);
                cursor.thumbs.push(thumb);
            }
            if (yAxisThumb(cursor)) {
                thumbCx = cursor.showThumbs.indexOf('l') !== -1 ? plotOffset.left - thumbRadius : plotOffset.left + plot.width() + thumbRadius;
