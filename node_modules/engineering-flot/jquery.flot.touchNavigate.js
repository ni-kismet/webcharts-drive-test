...
(function($) {
    ...
    var ZOOM_DISTANCE_MARGIN = $.plot.uiConstants.ZOOM_DISTANCE_MARGIN,
        SNAPPING_CONSTANT = $.plot.uiConstants.SNAPPING_CONSTANT;
    ...
    function initTouchNavigation(plot, options) {
        ...
        pinch = {
            ...
            drag: function(e) {
                ...
                gestureState.prevDistanceX = null;
                gestureState.prevDistanceY = null;
            }
        };
    ...
    function zoomPlot(plot, e, gestureState, navigationState) {
        ...
        //var zoomAxis = navigationState.touchedAxis ? navigationState.touchedAxis : getAxesForZoom(e.detail.touches[0], e.detail.touches[1], plot);
        var zoomAxis = navigationState.touchedAxis;
        ...
        if (zoomAxis) {
            ...
            gestureState.prevDistance = dist; //TODO change this. We should not need to update every time this for pan
            if ( ! (Math.abs(e.detail.touches[0].pageX - e.detail.touches[1].pageX) < SNAPPING_CONSTANT && Math.abs(e.detail.touches[0].pageY - e.detail.touches[1].pageY) >= SNAPPING_CONSTANT)) {
                dist = Math.abs(e.detail.touches[0].pageX - e.detail.touches[1].pageX);
                zoomAmount = dist / gestureState.prevDistanceX;
                plot.zoom({
                    center: center,
                    amount: zoomAmount,
                    axes: plot.getXAxes()
                });
                console.log(zoomAmount, dist, gestureState.prevDistanceX);
                gestureState.prevDistanceX = dist;
            }

            if (!(Math.abs(e.detail.touches[0].pageY - e.detail.touches[1].pageY) < SNAPPING_CONSTANT && Math.abs(e.detail.touches[0].pageX - e.detail.touches[1].pageX) >= SNAPPING_CONSTANT)) {
                dist = Math.abs(e.detail.touches[0].pageY - e.detail.touches[1].pageY);
                zoomAmount = dist / gestureState.prevDistanceY;
                plot.zoom({
                    center: center,
                    amount: zoomAmount,
                    axes: plot.getYAxes()
                });
                console.log(zoomAmount, dist, gestureState.prevDistanceY);
                gestureState.prevDistanceY = dist;
            }
        }
    }

    function getAxesForZoom(touch1, touch2, plot) {
        if (Math.abs(touch1.pageX - touch2.pageX) < SNAPPING_CONSTANT && Math.abs(touch1.pageY - touch2.pageY) >= SNAPPING_CONSTANT) {
            return plot.getYAxes();
        }

        if (Math.abs(touch1.pageY - touch2.pageY) < SNAPPING_CONSTANT && Math.abs(touch1.pageX - touch2.pageX) >= SNAPPING_CONSTANT) {
            return plot.getXAxes();
        }
    ...
    function setPrevDistance(e, gestureState) {
        ...
        gestureState.prevDistanceX = Math.abs(e.detail.touches[0].pageX - e.detail.touches[1].pageX);
        gestureState.prevDistanceY = Math.abs(e.detail.touches[0].pageY - e.detail.touches[1].pageY);
    }
